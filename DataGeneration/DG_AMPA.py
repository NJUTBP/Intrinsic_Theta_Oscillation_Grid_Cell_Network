...
Description: Generating data for Figure S6.
"""
...
# GAIList=[0.0,0.158,0.311,0.464,0.616,0.769]
# GNIList=[0.40,0.32,0.24,0.16,0.08,0.0]
# PerList=[0,20,40,60,80,100]
...
Parameters={'dt':dt,'T':T,
            ...
            'Mg':Mg,
            'GNmdaI':GNmdaI,'GAmpaI':GAmpaI,'GGabaE':GGabaE,'GGabaI':GGabaI,
...
def main(TrialID):
    ...
    for i in range(STEPs):
        ...
        #### calculating Ampa EI currents ####
        FSAmpa=np.fft.fft(SAmpa)
        FWEI_SAmpa=FWEI*FSAmpa
        SumAEI=np.real(np.fft.ifft(FWEI_SAmpa))[0::4]
        IAmpaI=(VI-ZE)*GAmpaI*SumAEI
        ...
        ILeakI=GLI*(VI-ZL)
        #### integration ####
        K1I=(-INmdaI-IAmpaI-IGabaI-IB2I-IMS2I-IN2I-ILeakI)/CMI
        VITemp=VI+dt*K1I
        ...
        #### calculating Ampa EI currents ####
        FSAmpaTemp=np.fft.fft(SAmpaTemp)
        FWEI_SAmpaTemp=FWEI*FSAmpaTemp
        SumAEITemp=np.real(np.fft.ifft(FWEI_SAmpaTemp))[0::4]
        IAmpaI=(VITemp-ZE)*GAmpaI*SumAEITemp
        #### calculating Gaba II currents ####
        ...
        #### integration ####
        K2I=(-INmdaI-IAmpaI-IGabaI-IB2I-IMS2I-IN2ITemp-ILeakI)/CMI
    ...
    np.savez_compressed(FilePath+'Result_'+str(TrialID)+'.npz',
        ...
        IEI=-CurrentEI/CurrentCount,IAI=-CurrentAI/CurrentCount,III=-CurrentII/CurrentCount,INI=-CurrentNI/CurrentCount,ILI=-CurrentLI/CurrentCount,IBI=-CurrentBI,IMS=-CurrentMS,
    ...
    ax6.plot(-CurrentAI/CurrentCount,ThetaI,label='IAI')
    ax6.plot((-CurrentEI/CurrentCount)+(-CurrentAI/CurrentCount)+(-CurrentII/CurrentCount)+(-CurrentBI-CurrentMS)+(-CurrentNI/CurrentCount),ThetaI,label='Itot')
    ax5.legend()
    ax6.legend()
