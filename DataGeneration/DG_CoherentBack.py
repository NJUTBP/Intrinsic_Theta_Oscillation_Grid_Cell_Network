...
Description: Generating data for Figure S2a-b, S6 in main text.
"""
#### import packages ####
import numpy as np
...
GExtE=3.1#nS
GExtI=2.38#nS
CommonRatio=float(sys.argv[1])
FextCommon=Fext*CommonRatio
FextIndependent=Fext*(1-CommonRatio)
...
SavePath='../Data/DG_CoherentBack/CR'+sys.argv[1]
isExist=os.path.exists(SavePath)
if not isExist: os.makedirs(SavePath)
np.savetxt(SavePath+'/Parameters.txt',np.array([DeltaT,T,NE,NI,CME,CMI,ZE,ZI,tauref_E,tauref_I,GLE,GLI,ZL,Eth,Eres,JEIpos,sigmaEI,muIE,sigmaIE,Mg,GNmdaI,GGabaE,GGabaI,TauAmpa,TauGaba,TauX,TauNmda,AlphaNmda,Fext,GExtE,GExtI,CommonRatio]))
...
def main():
    ...
    for i in range(STEPs):
        ...
        ######## background spike? ########
        FlagBCommon2EI=np.random.poisson(FextCommon*DeltaT)
        FlagB2E=np.random.poisson((FextIndependent*DeltaT),NE)
        FlagB2I=np.random.poisson((FextIndependent*DeltaT),NI)
        SExtE+=(FlagB2E+FlagBCommon2EI)
        SExtI+=(FlagB2I+FlagBCommon2EI)
    ...
    hist,binedges=np.histogram(SpikeInBump[:,0],bins=np.linspace(0,7000,700,endpoint=False))

    f, Pxx_den = signal.welch(hist, fs=100, window=signal.get_window('hamming',100), noverlap=50, nfft=1024)
