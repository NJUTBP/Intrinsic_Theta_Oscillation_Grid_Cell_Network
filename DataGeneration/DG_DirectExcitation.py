...
Description: Generating data for Figure S8.
"""
...
Stim2EEnd=500
IS2E0=np.zeros(NE)
IS2E0[int(NE/4)-int(NE/16):int(NE/4)+int(NE/16)]=0#pA
IS2E0[int(NE*3/4)-int(NE/16):int(NE*3/4)+int(NE/16)]=0#pA
...
Parameters={'dt':dt,'T':T,
            ...
            'JEEpos':JEEpos,'sigmaEE':sigmaEE,'JEIpos':JEIpos,'sigmaEI':sigmaEI,'muIE':muIE,'sigmaIE':sigmaIE,
            'Mg':Mg,
            'GNmdaE':GNmdaE,'GNmdaI':GNmdaI,'GGabaE':GGabaE,'GGabaI':GGabaI,
...
FilePath='../Data/DirectExcitation/'
...
def main(TrialID):
    ...
    SumExp=np.sum(np.exp(-IntegratePoints**2/(2*sigmaEE**2)))
    JEEneg=(10000-SumExp*JEEpos)/(10000-SumExp)
    DeltaTheta=(ThetaE-ThetaE[0])*np.append(np.ones(int(NE/2)),np.zeros(int(NE/2)))+(360-ThetaE+ThetaE[0])*np.append(np.zeros(int(NE/2)),np.ones(int(NE/2)))
    WEE=JEEneg+(JEEpos-JEEneg)*np.exp(-DeltaTheta**2/(2*sigmaEE**2))
    FWEE=np.fft.fft(WEE)
    #### defining WEI ####
    ...
    for i in range(STEPs):
        ...
        #### calculating Nmda EE currents ####
        FSNmda=np.fft.fft(SNmda)
        FWEE_SNmda=FWEE*FSNmda
        SumNEE=np.real(np.fft.ifft(FWEE_SNmda))
        INmdaE=(VE-ZE)*GNmdaE*SumNEE/(1.0+Mg*np.exp(-0.062*VE)/3.57)
        ...
        ILeakE=GLE*(VE-ZL)
        #### integration ####
        K1E=(IS2E-INmdaE-IGabaE-IB2E-IN2E-ILeakE)/CME
        VETemp=VE+dt*K1E
        ...
        FWEE_SNmdaTemp=FWEE*FSNmdaTemp
        SumNEETemp=np.real(np.fft.ifft(FWEE_SNmdaTemp))
        INmdaE=(VETemp-ZE)*GNmdaE*SumNEETemp/(1.0+Mg*np.exp(-0.062*VETemp)/3.57)
        #### calculating Gaba IE currents ####
        ...
        #### integration ####
        K2E=(IS2E-INmdaE-IGabaE-IB2E-IN2ETemp-ILeakE)/CME
