...
Description: Generating data for Figure S10.
"""
...
FBacCom=FBac*CommonRatio
FBacInd=FBac*(1-CommonRatio)
...
Parameters={'dt':dt,'T':T,
            ...
            'FBac':FBac,'GB2E':GB2E,'GB2I':GB2I,'CommonRatio':CommonRatio,
            'FMS':FMS,'GMS2I':GMS2I,
            'SamplingRate':SamplingRate,
            'Stim2EStart':Stim2EStart,'Stim2EEnd':Stim2EEnd,'IS2E0':IS2E0}

FilePath='../Data/PoissonBack/'
...
def main(TrialID):
    ...
    # Back2E=np.zeros((int(NE/8),TbinNum))
    # Back2I=np.zeros((int(NI/8),TbinNum))
    # MS2I=np.zeros((int(NI/8),TbinNum))
    ...
    for i in range(STEPs):
        ...
        IBackE=(VE-ZE)*GB2E*SB2E
        #### calculating leaky currents ####
        ILeakE=GLE*(VE-ZL)
        #### integration ####
        K1E=(IStimE-IGabaE-IBackE-ILeakE)/CME
        VETemp=VE+dt*K1E
        ...
        #### calculating background currents ####
        IBackI=(VI-ZE)*GB2I*SB2I
        #### calculating medial septal currents ####
        IMS2I=(VI-ZI)*GMS2I*SMS2I
        #### calculating leaky currents ####
        ILeakI=GLI*(VI-ZL)
        #### integration ####
        K1I=(-INmdaI-IGabaI-IBackI-IMS2I-ILeakI)/CMI
        VITemp=VI+dt*K1I
        ...
        #### calculating background currents ####
        IBackE=(VETemp-ZE)*GB2E*SB2ETemp
        #### calculating leaky currents ####
        ILeakE=GLE*(VETemp-ZL)
        #### integration ####
        K2E=(IStimE-IGabaE-IBackE-ILeakE)/CME
        ...
        #### calculating background currents ####
        IBackI=(VITemp-ZE)*GB2I*SB2ITemp
        #### calculating medial septal currents ####
        IMS2I=(VITemp-ZI)*GMS2I*SMS2ITemp
        #### calculating leaky currents ####
        ILeakI=GLI*(VITemp-ZL)
        #### integration ####
        K2I=(-INmdaI-IGabaI-IBackI-IMS2I-ILeakI)/CMI
        ...
        ######## background spike? ########
        FlagB2E=np.random.poisson((FBacInd*dt),NE)
        FlagB2I=np.random.poisson((FBacInd*dt),NI)
        FlagBCom2EI=np.random.poisson(FBacCom*dt)
        SB2E+=(FlagB2E+FlagBCom2EI)
        SB2I+=(FlagB2I+FlagBCom2EI)
        
        ######## medial septal spike? ########
        FlagMS2I=np.random.poisson((FMS*dt),NI)
        SMS2I+=FlagMS2I
        ...
        if i%(Gaba2E_RecordInterval)==0:
            ...
            # Back2E[:,int(i/Gaba2E_RecordInterval)]=IBackE[VmE_RecNeu_ID]
            # Back2I[:,int(i/Gaba2E_RecordInterval)]=IBackI[VmI_RecNeu_ID]
            # MS2I[:,int(i/Gaba2E_RecordInterval)]=IMS2I[VmI_RecNeu_ID]
    ...
    np.savez_compressed(FilePath+'Result_'+str(TrialID)+'.npz',
        ...
        IEE=-CurrentEE/CurrentCount,IIE=-CurrentIE/CurrentCount,IBE=-CurrentBE/CurrentCount,ILE=-CurrentLE/CurrentCount,
        IEI=-CurrentEI/CurrentCount,III=-CurrentII/CurrentCount,IBI=-CurrentBI/CurrentCount,IMS=-CurrentMS/CurrentCount,ILI=-CurrentLI/CurrentCount,
    ...
    ax5.plot(-CurrentBE/CurrentCount,ThetaE,label='IBE')
    ax5.plot((-CurrentEE/CurrentCount)+(-CurrentIE/CurrentCount)+(-CurrentBE/CurrentCount),ThetaE,label='Itot')
    ax6.plot(-CurrentEI/CurrentCount,ThetaI,label='IEI')
    ax6.plot(-CurrentII/CurrentCount,ThetaI,label='III')
    ax6.plot(-CurrentBI/CurrentCount,ThetaI,label='IBI')
    ax6.plot((-CurrentEI/CurrentCount)+(-CurrentII/CurrentCount)+(-CurrentBI/CurrentCount)+(-CurrentMS/CurrentCount),ThetaI,label='Itot')
    ax6.plot(-CurrentMS/CurrentCount,ThetaI,label='IMS')
    ax5.legend()
    ax6.legend()
