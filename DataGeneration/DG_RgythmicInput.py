...
Description: Generating data for Figure 7, S11, S12, S13.
"""
...
IB2I=-325.0#pA
IMS2I_Avg=100.0
IMS2I_Amp=100.0#pA#Amplitude of MS current
IMS2I_Rhy=8.0#Hz#Rhythm of MS current
IMS2I_Fra=1.0#Hz#Fraction of interneurons received MS current
...
IMS2INeu=np.append(np.ones(int(NI*IMS2I_Fra)),np.zeros(NI-int(NI*IMS2I_Fra))).astype(np.int32)
np.random.shuffle(IMS2INeu)
...
Stim2EEnd=500
IS2E0=np.zeros(NE)
IS2E0[int(NE/4)-int(NE/16):int(NE/4)+int(NE/16)]=200#pA
IS2E0[int(NE*3/4)-int(NE/16):int(NE*3/4)+int(NE/16)]=200#pA
Stim2IStart=0
Stim2IEnd=500
IS2I0=np.zeros(NI)
IS2I0[int(NI/4)-int(NI/16):int(NI/4)+int(NI/16)]=0#pA
IS2I0[int(NI*3/4)-int(NI/16):int(NI*3/4)+int(NI/16)]=0#pA
...
Parameters={'dt':dt,'T':T,
            ...
            'IB2E':IB2E,'IB2I':IB2I,'IMS2I_Avg':IMS2I_Avg,'IMS2I_Amp':IMS2I_Amp,'IMS2I_Rhy':IMS2I_Rhy,'IMS2I_Fra':IMS2I_Fra,
            'TauNoise':TauNoise,'SigmaNoise':SigmaNoise,
            'SamplingRate':SamplingRate,
            'Stim2EStart':Stim2EStart,'Stim2EEnd':Stim2EEnd,'IS2E0':IS2E0,'Stim2IStart':Stim2IStart,'Stim2IEnd':Stim2IEnd,'IS2I0':IS2I0}

FilePath='../Data/RhythmicInput/'
...
def main(TrialID):
    ...
    for i in range(STEPs):
        ...
        if Stim2EStart<(i*dt) and (i*dt)<Stim2EEnd:
            IS2E=IS2E0
        if Stim2IStart<(i*dt) and (i*dt)<Stim2IEnd:
            IS2I=IS2I0
        ...
        #### calculating leaky currents ####
        ILeakI=GLI*(VI-ZL)
        #### calculating MS currents ####
        IMS2I=IMS2I_Avg+IMS2INeu*IMS2I_Amp*np.cos(2*np.pi*IMS2I_Rhy*i*dt/1000)
        #### integration ####
        K1I=(IS2I-INmdaI-IGabaI-IB2I-IMS2I-IN2I-ILeakI)/CMI
        VITemp=VI+dt*K1I
        ...
        #### calculating MS currents ####
        IMS2I=IMS2I_Avg+IMS2INeu*IMS2I_Amp*np.cos(2*np.pi*IMS2I_Rhy*(i+1)*dt/1000)
        #### integration ####
        K2I=(IS2I-INmdaI-IGabaI-IB2I-IMS2I-IN2ITemp-ILeakI)/CMI
    ...
    MS2I_Phase=2*np.pi*IMS2I_Rhy*Tbins/1000
    MS2I=IMS2I_Avg+IMS2I_Amp*np.cos(MS2I_Phase)
    ...
    SGabaE2MS2I_PLI=np.zeros(2)
    DeltaPhaseMS=MS2I_Phase[200:]-Phase_SGabaE[200:]
    yMS=np.mean(np.sin(DeltaPhaseMS))
    xMS=np.mean(np.cos(DeltaPhaseMS))
    SGabaE2MS2I_PLI[0]=np.arctan2(yMS,xMS)
    SGabaE2MS2I_PLI[1]=np.sqrt(xMS**2+yMS**2)
    ...
    np.savez_compressed(FilePath+'Result_'+str(TrialID)+'.npz',
        ...
        IEI=-CurrentEI/CurrentCount,III=-CurrentII/CurrentCount,INI=-CurrentNI/CurrentCount,ILI=-CurrentLI/CurrentCount,IBI=-CurrentBI,IMS=-CurrentMS/CurrentCount,
        ...
        MS2I_Phase=MS2I_Phase,MS2I=MS2I,
        ...
        ESH2SGE_PLI=ESpike2SGabaE_PLI,ISH2SGE_PLI=ISpike2SGabaE_PLI,SGabaE2MS2I_PLI=SGabaE2MS2I_PLI
        )
    ...
    ShowVmEID=np.where(VmE_RecNeu_ID==256)[0][0]
    ShowVmIID=np.where(VmI_RecNeu_ID==64)[0][0]
    ...
    ax1.plot(PVs[:,0],PVs[:,1]/2)
    ax1.plot(Tbins,NormPVs[:,0]/2+180,color='tab:green')
    ax1.plot(Tbins,MS2I-IMS2I_Avg+180,color='gray',alpha=0.3)
    ...
    ax6.plot(-CurrentBI-CurrentMS/CurrentCount,ThetaI,label='IBI+IMS')
    ax6.plot(-CurrentNI/CurrentCount,ThetaI,label='INI')
    ax6.plot((-CurrentEI/CurrentCount)+(-CurrentII/CurrentCount)+(-CurrentBI-CurrentMS/CurrentCount)+(-CurrentNI/CurrentCount),ThetaI,label='Itot')
    ax5.legend()
    ax6.legend()
    ...
    axY.plot(ACbins[5:],ACEs[256,5:])
    axY.plot(ACbins[5:],ACIs[64,5:])
    axZ.scatter(PhaseLockingIndexE[256-64:256+64,0],PhaseLockingIndexE[256-64:256+64,1])
    axA.scatter(PhaseLockingIndexI[64-16:64+16,0],PhaseLockingIndexI[64-16:64+16,1])
    axB.scatter(ESpike2SGabaE_PLI[0],ESpike2SGabaE_PLI[1])
    axB.scatter(ISpike2SGabaE_PLI[0],ISpike2SGabaE_PLI[1])
    axB.scatter(SGabaE2MS2I_PLI[0],SGabaE2MS2I_PLI[1])
    ax1.set_xlim(0,T)
    ax1.set_ylim(0,360)
