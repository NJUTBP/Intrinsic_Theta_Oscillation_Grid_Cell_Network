...
Description: Generating data for Figure S9.
"""
...
Parameters={'dt':dt,'T':T,
            ...
            'JEIpos':JEIpos,'sigmaEI':sigmaEI,'muIE':muIE,'sigmaIE':sigmaIE,'muII':muII,'sigmaII':sigmaII,
            'Mg':Mg,
...
def main(TrialID):
    ...
    #### defining WII ####
    SumExp=np.sum(np.exp(-(IntegratePoints-muII)**2/2/sigmaII**2)*180/10000)
    GII=180/SumExp
    DeltaTheta=(ThetaI-ThetaI[0])*np.append(np.ones(int(NI/2)),np.zeros(int(NI/2)))+(360-ThetaI+ThetaI[0])*np.append(np.zeros(int(NI/2)),np.ones(int(NI/2)))
    WII=GII*np.exp(-(DeltaTheta-muII)**2/2/sigmaII**2)
    FWII=np.fft.fft(WII)
    # print(np.sum(WII),np.sum(WEI))
    # fig,ax=plt.subplots()
    # ax.plot(np.arange(NI),WII)
    # plt.show()
    ...
    for i in range(STEPs):
        ...
        FWII_SGaba=FWII*FSGaba
        SumGII=np.real(np.fft.ifft(FWII_SGaba))
        # SumGII=np.sum(SGaba)
        ...
        #### calculating Gaba II currents ####
        FWII_SGabaTemp=FWII*FSGabaTemp
        SumGIITemp=np.real(np.fft.ifft(FWII_SGabaTemp))
